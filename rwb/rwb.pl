...
my $rep_dem_money =0;#pos if dem, neg if rep
print "div id='rep_dem_money'>$rep_dem_money</div>";
#
...
if ($action eq "base") { 
  ...
  print "<form><input type=\"checkbox\" id=\"committee\" onclick='ViewShift()' >Committees</input><br><input type=\"checkbox\" onclick='ViewShift()'  id=\"candidate\" >Candidates</input><br><input type=\"checkbox\" id=\"individual\" onclick='ViewShift()'>Individuals</input><br><input type=\"checkbox\" onclick='ViewShift()'  id=\"opinion\" >Opinions</input></form>";
  #
  my @allCycles;

 eval { @allCycles = ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.candidate_master");};

 print "<div id='cycleType' style='float:right'>";
 foreach (@allCycles) {
   print "<input type='checkbox' id=@{$_} class='cycles' onclick='ViewShift()'>@{$_} <br>"; 
 }
 print "</div>";
  #
...
if ($action eq "near") {
  ...
  if ($what{committees}) { 
    ...
    my @CommitteeMoney_dem = getMoneyCommittee_dem($latne,$longne,$latsw,$longsw,$cycle,$format);
    my @CommitteeMoney_rep = getMoneyCommittee_rep($latne,$longne,$latsw,$longsw,$cycle,$format);
    $rep_dem_money+=$CommitteeMoney_dem[0];
    $rep_dem_money-=$CommitteeMoney_rep[0];
print "</div>";
    if (!$error) {
      if ($format eq "table") { 
  ...
  if ($what{individuals}) {
    ...
    my @individualMoney_dem = getMoneyIndividuals_dem($latne,$longne,$latsw,$longsw,$cycle,$format);
    my @individualMoney_rep = getMoneyIndividuals_rep($latne,$longne,$latsw,$longsw,$cycle,$format);
    $rep_dem_money+=$individualMoney_dem[0];
    $rep_dem_money-=$individualMoney_rep[0];
  ...
  if ($what{opinions}) {
    ...
    my @opinionwinner = getOpinionsWinner($latne,$longne,$latsw,$longsw,$cycle,$format);
    $rep_dem_money = @opinionwinner[0]; 
    if (!$error) {
      if ($format eq "table") { 
...
#
#(b.cmte_pty_affiliation = 'REP' OR b.cmte_pty_affiliation = 'DEM') and

sub getMoneyCommittee_dem{
my ($latne,$longne,$latsw,$longsw,$cycle,$format) = @_;

 my $dem = "select sum(a.transaction_amnt) ";
 $dem .= "from cs339.comm_to_comm a,  cs339.committee_master b, cs339.cmte_id_to_geo c , cs339.comm_to_cand d ";
 $dem .= "where a.cmte_id = b.cmte_id and b.cmte_id = c.cmte_id and c.cmte_id = d.cmte_id and (b.cmte_pty_affiliation='DEM' or b.cmte_pty_affiliation='Dem') and b.cycle in ($cycle) and c.latitude>$latsw and c.latitude<$latne and c.longitude>$longsw and c.longitude<$longne ";
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, $dem);
  };
}

sub getMoneyCommittee_rep{
my ($latne,$longne,$latsw,$longsw,$cycle,$format) = @_;
 my $rep = "select  sum(a.transaction_amnt) ";
 $rep .= "from cs339.comm_to_cand a,  cs339.committee_master b, cs339.cmte_id_to_geo c, cs339.comm_to_comm d ";
 $rep .= "where a.cmte_id = b.cmte_id and b.cmte_id = c.cmte_id and c.cmte_id = d.cmte_id and (b.cmte_pty_affiliation='REP' or b.cmte_pty_affiliation='Rep') and b.cycle in ($cycle) and c.latitude>$latsw and c.latitude<$latne and c.longitude>$longsw and c.longitude<$longne ";

  my @rows;

  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, $rep);;
  };
}


sub getMoneyIndividuals_dem{
my ($latne,$longne,$latsw,$longsw,$cycle,$format) = @_;
   my $rep = "select sum(a.transaction_amnt) ";
   $rep .= "from cs339.individual a,  cs339.committee_master b, cs339.ind_to_geo c ";
   $rep .= "where a.cmte_id = b.cmte_id and (b.cmte_pty_affiliation='REP' or b.cmte_pty_affiliation='Rep') and b.cycle in ($cycle) and c.latitude>$latsw and c.latitude<$latne and c.longitude>$longsw and c.longitude<$longne ";
...
sub getOpinionsWinner{ #POS IF DEM, NEG IF REP 
  my ($latne,$longne,$latsw,$longsw,$cycle,$format) = @_;
  my $sql = "select sum(color) ";
  $sql .= "from rwb_opinions a ";
  $sql .= "where a.latitude>$latsw and a.latitude<$latne and a.longitude>$longsw and a.longitude<$longne";
  my @rows;
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, $sql);
  };
}
